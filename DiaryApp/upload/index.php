...
if(isset($_GET['id'])) {
    $id = (int) $_GET['id'];
    $prepare = $connection->prepare("SELECT title, weekend, content FROM notes WHERE id = ?");
...
<div class="flex-grow flex flex-col items-center w-[90vw] xl:w-[70vw] 2xl:w-[60vw] mt-10">
    ...
    <div class="flex flex-col items-center bg-green-50/70 rounded-2xl w-full shadow shadow-lg shadow-neutral-800/70">
        <h2 class="font-extrabold uppercase mt-16 text-5xl sm:text-6xl xl:text-7xl"><?php echo (!empty($id)) ? 'Edit' : 'Add';?> Memory</h2>

        <form action="post.php<?php if (!empty($id)) echo e('?'.query('id', $id)); ?>" method="post" enctype="multipart/form-data" class="flex vapitalize text-neutral-800 font-bold text-xl flex-col gap-y-3 w-[75%] md:w-[60%] py-10">
            <label for="title">Title</label>
            <input type="text" maxlength="30" name="title" id="title" placeholder="My Title" required
                    <?php if (!empty($data)): ?> value="<?php echo $data[0]['title'];?>" <?php endif; ?>>
            <label for="weekend">Weekend</label>
            <input type="number" name="weekend" id="weekend" min="1" max="4" placeholder="2" required
                <?php if (!empty($data)): ?> value="<?php echo $data[0]['weekend'];?>" <?php endif; ?>"/>
            <label for="content">Content</label>
            <textarea name="content" id="content" maxlength="200" cols="10" rows="5" placeholder="Lorem Ipsum" required

            ><?php if (!empty($data)) echo $data[0]['content']; ?> </textarea>
